...
class ResDepSepBlock(nn.Module):
    def __init__(self, C_in, C_out, kernel_size, stride,ratio=6):
        ...
        self.net = nn.Sequential(
            ...
            nn.Conv1d(hidden_dim,hidden_dim,kernel_size=kernel_size,stride=stride,padding=padding,groups=hidden_dim,bias=False),
            nn.BatchNorm1d(hidden_dim),
            nn.ReLU6(inplace=True),
...
class OneD_ResCNN(nn.Module):
    def __init__(self, input_size,channel):
        ...
        self.net = nn.Sequential(
            ResDepSepBlock(channel,channel*2,kernel_size=5,stride=4),
            ResDepSepBlock(channel*2,channel*2,kernel_size=3,stride=1),
            ResDepSepBlock(channel*2,channel*2,kernel_size=3,stride=1),

            ResDepSepBlock(channel*2,channel*4,kernel_size=5,stride=4),
